...
function startRocketTag(){
    ...
    const keys={};let p1,p2,chaser=1,score1=0,score2=0,winner=null,animationId,lastTime=performance.now(),phase=0,burst=[];
    function resetPositions(){p1={x:canvas.width*.25,y:canvas.height/2,r:22,color:"#67e8f9",vx:0,vy:0};p2={x:canvas.width*.75,y:canvas.height/2,r:22,color:"#fb7185",vx:0,vy:0};burst=[];}resetPositions();
    function keyDown(e){keys[e.key]=true;}function keyUp(e){keys[e.key]=false;}
    function update(dt){if(winner)return;phase+=dt;for(const [p,up,down,left,right,dash] of [[p1,"w","s","a","d"," "],[p2,"ArrowUp","ArrowDown","ArrowLeft","ArrowRight","Enter"]]){let dx=(keys[right]?1:0)-(keys[left]?1:0),dy=(keys[down]?1:0)-(keys[up]?1:0),boost=keys[dash]?1.8:1;p.x+=dx*300*boost*dt;p.y+=dy*300*boost*dt;p.x=Math.max(35,Math.min(canvas.width-35,p.x));p.y=Math.max(85,Math.min(canvas.height-35,p.y));}const a=chaser===1?p1:p2,b=chaser===1?p2:p1;if(Math.hypot(a.x-b.x,a.y-b.y)<a.r+b.r){if(chaser===1)score1++;else score2++;burst.push({x:b.x,y:b.y,life:1});if(score1>=5||score2>=5)winner=score1>score2?"PLAYER 1":"PLAYER 2";else{chaser=chaser===1?2:1;resetPositions();}}for(let i=burst.length-1;i>=0;i--){burst[i].life-=dt*2;if(burst[i].life<=0)burst.splice(i,1);}}
    function draw(){drawGameAtmosphere(ctx,canvas,"#ec4899",phase*25);ctx.strokeStyle="rgba(236,72,153,.3)";ctx.lineWidth=3;ctx.strokeRect(35,75,canvas.width-70,canvas.height-110);for(let i=0;i<6;i++){const x=(i+1)*canvas.width/7,y=120+Math.sin(phase*2+i)*40;ctx.fillStyle="rgba(236,72,153,.16)";ctx.beginPath();ctx.arc(x,y,28,0,Math.PI*2);ctx.fill();}for(const p of [p1,p2]){ctx.save();ctx.shadowColor=p.color;ctx.shadowBlur=28;ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fill();ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(p.x-7,p.y-8,5,0,Math.PI*2);ctx.fill();ctx.restore();}const hunter=chaser===1?p1:p2;ctx.strokeStyle=hunter.color;ctx.globalAlpha=.35;ctx.lineWidth=5;ctx.beginPath();ctx.arc(hunter.x,hunter.y,hunter.r+12+Math.sin(phase*8)*4,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1;for(const b of burst){ctx.globalAlpha=b.life;ctx.strokeStyle="#fff";ctx.lineWidth=4;ctx.beginPath();ctx.arc(b.x,b.y,60*(1-b.life),0,Math.PI*2);ctx.stroke();}ctx.globalAlpha=1;ctx.textAlign="center";ctx.font="900 38px Arial";ctx.fillStyle="#67e8f9";ctx.fillText(score1,canvas.width/2-80,58);ctx.fillStyle="#fb7185";ctx.fillText(score2,canvas.width/2+80,58);ui.innerHTML="🚀 ROCKET TAG<br><small>"+(chaser===1?"Player 1":"Player 2")+" is chasing • WASD / Arrows</small>";if(winner){ctx.fillStyle="rgba(0,0,0,.76)";ctx.fillRect(0,0,canvas.width,canvas.height);ctx.fillStyle="#fff";ctx.font="900 48px Arial";ctx.fillText(winner+" WINS!",canvas.width/2,canvas.height/2-20);ctx.font="bold 18px Arial";ctx.fillText("Tap to rematch",canvas.width/2,canvas.height/2+25);}}
